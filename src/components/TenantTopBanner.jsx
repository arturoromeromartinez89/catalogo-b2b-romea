const ICON_PATHS = {
  whatsapp: (
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z" />
  ),
  facebook: (
    <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047v-2.66c0-3.026 1.792-4.697 4.533-4.697 1.313 0 2.686.235 2.686.235v2.971H15.83c-1.491 0-1.956.93-1.956 1.886v2.265h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z" />
  ),
  tiktok: (
    <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
  ),
  linkedin: (
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM7.119 20.452H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z" />
  ),
};

export const SocialIcon = ({ name, size = 15 }) => {
  if (name === "website") {
    return (
      <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden="true" focusable="false">
        <circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" strokeWidth="2" />
        <path
          d="M3 12h18M12 3c2.5 2.6 3.9 5.7 3.9 9s-1.4 6.4-3.9 9c-2.5-2.6-3.9-5.7-3.9-9S9.5 5.6 12 3z"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
        />
      </svg>
    );
  }
  if (name === "instagram") {
    return (
      <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden="true" focusable="false">
        <rect x="2" y="2" width="20" height="20" rx="5.5" fill="none" stroke="currentColor" strokeWidth="2" />
        <circle cx="12" cy="12" r="4.4" fill="none" stroke="currentColor" strokeWidth="2" />
        <circle cx="17.3" cy="6.7" r="1.4" fill="currentColor" />
      </svg>
    );
  }
  const path = ICON_PATHS[name];
  if (!path) return null;
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} fill="currentColor" aria-hidden="true" focusable="false">
      {path}
    </svg>
  );
};

const SOCIAL_ITEMS = [
  { key: "website", showKey: "show_website", label: "Sitio web" },
  { key: "instagram", showKey: "show_instagram", label: "Instagram" },
  { key: "facebook", showKey: "show_facebook", label: "Facebook" },
  { key: "tiktok", showKey: "show_tiktok", label: "TikTok" },
  { key: "linkedin", showKey: "show_linkedin", label: "LinkedIn" },
];

const ensureUrl = (value = "") => {
  const text = String(value || "").trim();
  if (!text) return "";
  if (/^https?:\/\//i.test(text)) return text;
  return `https://${text.replace(/^\/+/, "")}`;
};

const phoneHref = (value = "") => {
  const digits = String(value || "").replace(/\D/g, "");
  return digits.length >= 8 ? `tel:${digits}` : "";
};

const whatsappHref = (number = "", message = "") => {
  const digits = String(number || "").replace(/\D/g, "");
  if (digits.length < 8) return "";
  const normalizedNumber = digits.length === 10 ? `52${digits}` : digits;
  const text = String(message || "Hola, quiero apoyo con mi pedido.").trim();
  return `https://wa.me/${normalizedNumber}?text=${encodeURIComponent(text)}`;
};

export default function TenantTopBanner({ config = {}, compact = false }) {
  const announcement = config?.announcement || {};
  if (!announcement.enabled) return null;

  const contact = announcement.contact || {};
  const socials = announcement.socials || {};
  const text = String(announcement.text || "").trim();
  const contactItems = [];

  const whatsappUrl = whatsappHref(contact.whatsapp_number, contact.whatsapp_message);
  if (contact.show_whatsapp && whatsappUrl) {
    contactItems.push({
      key: "whatsapp",
      label: <SocialIcon name="whatsapp" />,
      title: "WhatsApp",
      href: whatsappUrl,
      external: true,
    });
  }

  const telUrl = phoneHref(contact.phone);
  if (contact.show_phone && telUrl) {
    contactItems.push({ key: "phone", label: contact.phone, title: "Llamar", href: telUrl });
  }

  if (contact.show_email && contact.email) {
    contactItems.push({ key: "email", label: contact.email, title: "Correo", href: `mailto:${contact.email}` });
  }

  const socialItems = SOCIAL_ITEMS
    .map((item) => ({
      ...item,
      href: socials[item.showKey] ? ensureUrl(socials[item.key]) : "",
    }))
    .filter((item) => item.href);

  if (!text && !contactItems.length && !socialItems.length) return null;
  const classes = [
    "tenant-top-banner",
    compact ? "tenant-top-banner--compact" : "",
    contactItems.length ? "" : "tenant-top-banner--no-contacts",
    text ? "" : "tenant-top-banner--no-ticker",
    socialItems.length ? "" : "tenant-top-banner--no-socials",
  ].filter(Boolean).join(" ");

  return (
    <div className={classes}>
      {contactItems.length ? <div className="tenant-top-banner__contacts">
        {contactItems.map((item) => (
          <a
            className={`tenant-top-banner__pill tenant-top-banner__pill--${item.key}`}
            href={item.href}
            key={item.key}
            title={item.title}
            aria-label={item.title}
            target={item.external ? "_blank" : undefined}
            rel={item.external ? "noreferrer" : undefined}
          >
            {item.label}
          </a>
        ))}
      </div> : null}

      {text ? (
        <div className={`tenant-top-banner__ticker${announcement.animate ? " is-animated" : ""}`} aria-label={text}>
          <span>{text}</span>
        </div>
      ) : null}

      {socialItems.length ? <div className="tenant-top-banner__socials">
        {socialItems.map((item) => (
          <a
            className="tenant-top-banner__social"
            href={item.href}
            key={item.key}
            target="_blank"
            rel="noreferrer"
            title={item.label}
            aria-label={item.label}
          >
            <SocialIcon name={item.key} />
          </a>
        ))}
      </div> : null}
    </div>
  );
}
