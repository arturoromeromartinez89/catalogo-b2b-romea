...
function PreorderEditorContent({ preorder: initial, clients, products = [], onClose, onSaved, onDirty, pricingLocked = false, tenantId = "", profile }) {
  ...
  const addProduct = (product) => {
    ...
    const nextItem = isConfigurableProductGroup(product)
      ...
      : priceItemFromLines(buildPreorderItemFromProduct(product, 1, lines, plataFinaMxn), lines, selectedList, getListSilverMxn(selectedList));
    setItems((current) => {
  ...
  const applyConfigurableVariant = (idx, variantCode) => {
    ...
    const pricedItem = priceItemFromLines(rawItem, lines, selectedList, getListSilverMxn(selectedList));
