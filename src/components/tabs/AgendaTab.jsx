...
export default function AgendaTab({ tenantId = "", profile = {}, clients = [] }) {
  ...
  const salesBudgetCards = useMemo(
    () => [
      {
        id: "annual",
        label: t("agAnnualBudget"),
        current: "$0",
        target: "$0",
        progress: 0,
        caption: t("agBudgetSourcePending"),
      },
      {
        id: "monthly",
        label: t("agMonthlyBudget"),
        current: "$0",
        target: "$0",
        progress: 0,
        caption: t("agBudgetSourcePending"),
  ...
  return (
    <section className="admin-workspace agenda-workspace">
      ...
      <section className="admin-soft-panel compact-panel agenda-budget-panel" aria-label={t("agBudgetTitle")}>
        <div className="agenda-budget-copy">
          <span className="tool-eyebrow">{t("agBudgetEyebrow")}</span>
          <h3>{t("agBudgetTitle")}</h3>
          <p className="muted">{t("agBudgetHint")}</p>
        </div>
        <div className="agenda-budget-grid">
          {salesBudgetCards.map((card) => (
            <article className="agenda-budget-card" key={card.id}>
              <div className="agenda-budget-card__head">
                <span>{card.label}</span>
                <strong>{card.progress}%</strong>
              </div>
              <div className="agenda-budget-values">
                <div>
                  <small>{t("agBudgetSold")}</small>
                  <b>{card.current}</b>
                </div>
                <div>
                  <small>{t("agBudgetGoal")}</small>
                  <b>{card.target}</b>
                </div>
              </div>
              <div className="agenda-budget-progress" aria-hidden="true">
          ...
          return (
            <section className={`agenda-column${isToday ? " agenda-column--today" : ""}`} key={dayKey}>
              <header>
                ...
                <small className="agenda-day-count">{t("agTasksShort", dayTasks.length)}</small>
              ...
              <div className="agenda-column-body">
                {dayTasks.map((task) => {
                  ...
                  return (
                    <article className={`agenda-task agenda-task--${task.status} agenda-task--${task.category}${overdue ? " agenda-task--overdue" : ""}`} key={task.id}>
                      ...
                      <div className="agenda-task-attachments">
                        <div className="agenda-task-attachments__head">
                          <span>{t("agAttachmentsCount", attachments.length)}</span>
                          <small>{t("agAttachmentVisualOnly")}</small>
                        </div>
                        {attachments.length ? (
                          <div className="agenda-attachment-list">
                            {attachments.map((file) => (
                              <span className="agenda-attachment-chip" key={file.id}>
                                <span>
                                ...
                                <button
                                  type="button"
                                  className="agenda-attachment-remove"
                          ...
                          <p className="agenda-attachment-empty">{t("agNoAttachments")}</p>
                        )}
                        <label className="agenda-attachment-button">
                          {t("agAttachFile")}
                          <input
                            type="file"
                            multiple
