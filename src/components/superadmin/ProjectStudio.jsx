  ...
  return <section className="ph-manager ph-studio">
    <header className="ph-manager__header">
      <div>
        ...
        <span>Proyectos de este cliente. El cliente solo ve los que estén publicados.</span>
      </div>
    ...
    <div className="ph-manager__layout">
      <aside className="ph-manager__projects">
        <div className="ph-manager__aside-head"><strong>Proyectos</strong><span>{projects.length}</span></div>
        {loading ? <p className="muted">Cargando...</p> : null}
        {projects.map((project) => <button className={selectedProjectId === project.id ? "active" : ""} type="button" key={project.id} onClick={() => selectProject(project)}>
          <strong>{project.name}</strong>
          <span>{projectStatusLabel(project.status)} · {confirmedProgress(project.project_deliverables)}% confirmado</span>
          <small>{project.published ? "Visible para el cliente" : "Borrador interno"}</small>
        </button>)}
        {!loading && !projects.length ? <p className="muted">Este cliente todavía no tiene proyectos. Crea el primero.</p> : null}
      </aside>

      <div className="ph-manager__content">
        <article className="ph-manager__card">
          <div className="ph-manager__card-head">
            <div><span>{selectedProjectId ? "Ficha del proyecto" : "Proyecto nuevo"}</span><h3>{projectDraft.name || "Sin nombre todavía"}</h3></div>
            <label className="ph-manager__publish">
              <input type="checkbox" checked={Boolean(projectDraft.published)} onChange={(event) => update("published", event.target.checked)} />
              <span>{projectDraft.published ? "Publicado" : "Borrador"}</span>
            </label>
          </div>

          <div className="ph-manager__form-grid">
            <label className="ph-manager__span-2">Nombre del proyecto<input value={projectDraft.name || ""} onChange={(event) => update("name", event.target.value)} placeholder="Ej. Digitalización de operaciones" /></label>
            <label className="ph-manager__span-2">Descripción para el cliente<textarea value={projectDraft.description || ""} onChange={(event) => update("description", event.target.value)} placeholder="Una o dos frases que el cliente entienda sin lenguaje técnico." /></label>
            <label className="ph-manager__span-2">Objetivo<textarea value={projectDraft.objective || ""} onChange={(event) => update("objective", event.target.value)} placeholder="Qué problema resuelve este proyecto." /></label>
            <label className="ph-manager__span-2">Meta<textarea value={projectDraft.goal || ""} onChange={(event) => update("goal", event.target.value)} placeholder="Cómo sabremos que se logró. Algo medible." /></label>
            <label className="ph-manager__span-2">Alcance incluido<textarea className="ph-studio__list-field" value={listToText(projectDraft.included_scope)} onChange={(event) => update("included_scope", event.target.value.split("\n"))} placeholder={"Un compromiso por línea\nInventario\nEntradas y salidas"} /><small>Una línea por punto.</small></label>
            <label className="ph-manager__span-2">Exclusiones<textarea className="ph-studio__list-field" value={listToText(projectDraft.excluded_scope)} onChange={(event) => update("excluded_scope", event.target.value.split("\n"))} placeholder={"Lo que NO incluye\nFacturación electrónica"} /><small>Una línea por punto. Evita malentendidos después.</small></label>
            <label>Fecha de inicio<input type="date" value={projectDraft.start_date || ""} onChange={(event) => update("start_date", event.target.value)} /></label>
            ...
            <label>Etapa actual<input value={projectDraft.current_phase_name || ""} onChange={(event) => update("current_phase_name", event.target.value)} placeholder="Ej. Diseño funcional" /></label>
            ...
            <label>Responsable interno<input value={projectDraft.internal_owner_name || ""} onChange={(event) => update("internal_owner_name", event.target.value)} /></label>
          </div>
        ...
        <article className="ph-manager__card">
          <div className="ph-manager__card-head">
            <div><span>Equipo</span><h3>Quién trabaja en este proyecto</h3></div>
          </div>
          {!selectedProject ? <p className="muted">Guarda el proyecto y después asigna a su equipo.</p> : <>
            <div className="ph-studio__team">
              {members.map((member) => {
                const person = profiles.find((item) => item.id === member.profile_id);
                return <div className="ph-studio__member" key={member.id}>
                  <i>{personName(person).slice(0, 2).toUpperCase()}</i>
                  <div><strong>{personName(person)}</strong><small>{person?.email || "Cuenta retirada"}</small></div>
                  <span>{memberRoleOptions.find((item) => item.value === member.project_role)?.label || member.project_role}</span>
                  <button type="button" className="danger" disabled={saving} onClick={() => dropMember(member)}>Retirar</button>
                </div>;
              })}
              {!members.length ? <p className="muted">Nadie asignado todavía.</p> : null}
            </div>
            <div className="ph-studio__team-add">
              <label>Agregar a<select value={newMember.profileId} onChange={(event) => setNewMember((current) => ({ ...current, profileId: event.target.value }))}>
                <option value="">Seleccionar persona</option>
                {availableStaff.map((person) => <option key={person.id} value={person.id}>{personName(person)} · {person.email}</option>)}
              </select></label>
              <label>Papel<select value={newMember.projectRole} onChange={(event) => setNewMember((current) => ({ ...current, projectRole: event.target.value }))}>
                {memberRoleOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
              </select></label>
              <button className="secondary-button compact-action" type="button" disabled={saving || !newMember.profileId} onClick={addMember}>Agregar</button>
            </div>
            {!availableStaff.length && members.length ? <p className="muted">Todo el equipo interno ya está en este proyecto.</p> : null}
          </>}
        </article>

        {selectedProject ? <article className="ph-manager__card ph-studio__summary">
          <div className="ph-manager__card-head"><div><span>Resumen</span><h3>Cómo se ve hoy</h3></div></div>
          <dl>
            <div><dt>Etapa</dt><dd>{selectedProject.current_phase_name || "Por definir"}</dd></div>
            <div><dt>Inicio</dt><dd>{formatDate(selectedProject.start_date)}</dd></div>
            <div><dt>Fecha objetivo</dt><dd>{formatDate(selectedProject.estimated_end_date)}</dd></div>
            <div><dt>Soluciones</dt><dd>{selectedProject.project_solutions?.length || 0}</dd></div>
            <div><dt>Equipo</dt><dd>{members.length}</dd></div>
            <div><dt>Visibilidad</dt><dd>{selectedProject.published ? "El cliente ya lo ve" : "Solo interno"}</dd></div>
          </dl>
        </article> : null}
      </div>
    </div>
