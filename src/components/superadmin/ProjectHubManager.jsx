...
const sections = [
  { id: "project_objectives", label: "Objetivos" },
...
const emptyChild = {
  ...
  project_tasks: { title: "", description: "", objective_id: "", phase_id: "", status: "todo", priority: "medium", start_date: "", due_date: "", progress_percentage: 0, assignee_name: "", sort_order: 10, visible_to_client: true, client_can_comment: true, client_can_upload: true, client_can_move: true },
...
const statusLabel = { draft: "Borrador", active: "Activo", planned: "Programado", at_risk: "En riesgo", on_hold: "En pausa", completed: "Completado", cancelled: "Cancelado", backlog: "Backlog", todo: "Por hacer", pending: "Pendiente", in_progress: "En curso", review: "En revisión", done: "Completada", blocked: "Bloqueado", delivered: "Entregado", approved: "Aprobado", rejected: "Rechazado", resolved: "Resuelto" };
...
function ChildForm({ table, draft, setDraft, project, saving, onSave, onCancel }) {
  ...
  return <div className="ph-manager__child-form"><h4>{draft.id ? "Editar registro" : "Agregar registro"}</h4>
    ...
    {table === "project_objectives" ? <><label>Nombre del periodo<input value={draft.period_label || ""} placeholder="Ej. 10–21 agosto" onChange={(event) => update("period_label", event.target.value)} /></label><div className="ph-manager__inline"><label>Inicio<input type="date" value={draft.period_start || ""} onChange={(event) => update("period_start", event.target.value)} /></label><label>Fin<input type="date" value={draft.period_end || ""} onChange={(event) => update("period_end", event.target.value)} /></label></div><label>Estado<select value={draft.status} onChange={(event) => update("status", event.target.value)}><option value="planned">Programado</option><option value="active">En curso</option><option value="completed">Completado</option><option value="at_risk">En riesgo</option></select></label><div className="ph-manager__inline"><label>Orden<input type="number" value={draft.sort_order ?? 0} onChange={(event) => update("sort_order", Number(event.target.value))} /></label><label>Avance<input type="number" min="0" max="100" value={draft.progress_percentage ?? 0} onChange={(event) => update("progress_percentage", Number(event.target.value))} /></label></div></> : null}
    {table === "project_tasks" ? <><label>Objetivo<select value={draft.objective_id || ""} onChange={(event) => update("objective_id", event.target.value)}><option value="">Sin objetivo</option>{(project?.project_objectives || []).map((item) => <option value={item.id} key={item.id}>{item.title}</option>)}</select></label><label>Etapa del roadmap<select value={draft.phase_id || ""} onChange={(event) => update("phase_id", event.target.value)}><option value="">Sin etapa</option>{(project?.project_phases || []).map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><div className="ph-manager__inline"><label>Estado<select value={draft.status} onChange={(event) => update("status", event.target.value)}><option value="backlog">Backlog</option><option value="todo">Por hacer</option><option value="in_progress">En curso</option><option value="review">En revisión</option><option value="blocked">Bloqueada</option><option value="done">Completada</option></select></label><label>Prioridad<select value={draft.priority} onChange={(event) => update("priority", event.target.value)}><option value="low">Baja</option><option value="medium">Media</option><option value="high">Alta</option><option value="critical">Crítica</option></select></label></div><div className="ph-manager__inline"><label>Inicio<input type="date" value={draft.start_date || ""} onChange={(event) => update("start_date", event.target.value)} /></label><label>Entrega<input type="date" value={draft.due_date || ""} onChange={(event) => update("due_date", event.target.value)} /></label></div><label>Responsable<input value={draft.assignee_name || ""} onChange={(event) => update("assignee_name", event.target.value)} /></label><div className="ph-manager__inline"><label>Orden<input type="number" value={draft.sort_order ?? 0} onChange={(event) => update("sort_order", Number(event.target.value))} /></label><label>Avance<input type="number" min="0" max="100" value={draft.progress_percentage ?? 0} onChange={(event) => update("progress_percentage", Number(event.target.value))} /></label></div><label className="ph-manager__check"><input type="checkbox" checked={draft.client_can_move !== false} onChange={(event) => update("client_can_move", event.target.checked)} /><span>El cliente puede mover esta tarjeta</span></label><label className="ph-manager__check"><input type="checkbox" checked={draft.client_can_comment !== false} onChange={(event) => update("client_can_comment", event.target.checked)} /><span>El cliente puede comentar</span></label><label className="ph-manager__check"><input type="checkbox" checked={draft.client_can_upload !== false} onChange={(event) => update("client_can_upload", event.target.checked)} /><span>El cliente puede adjuntar archivos</span></label></> : null}
