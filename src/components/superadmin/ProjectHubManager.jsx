...
const secondaryViews = [
  { id: "tasks", label: "Tareas", table: "project_tasks" },
...
const viewCopy = {
  ...
  tasks: ["Tareas", "Acciones concretas con responsable, fecha, entregable, dependencia y evidencia."],
...
const emptyChild = {
  ...
  project_tasks: { title: "", description: "", solution_id: "", deliverable_id: "", depends_on_task_id: "", status: "todo", priority: "medium", start_date: "", due_date: "", estimated_hours: "", progress_percentage: 0, assignee_name: "", repository_url: "", repository_label: "", branch_name: "", sort_order: 10, visible_to_client: true, client_can_comment: true, client_can_upload: true, client_can_move: true },
...
function ChildForm({ table, draft, setDraft, project, saving, onSave, onCancel }) {
  ...
  return <div className="ph-manager__child-form"><h4>{draft.id ? "Editar registro" : "Agregar registro"}</h4>
    ...
    {table === "project_solutions" ? <><label>Estado<select value={draft.status} onChange={(event) => update("status", event.target.value)}><option value="draft">Borrador</option><option value="planned">Por iniciar</option><option value="in_progress">En proceso</option><option value="waiting">En espera</option><option value="needs_changes">Requiere cambios</option><option value="completed">Terminada</option><option value="cancelled">Cancelada</option></select></label><label>Objetivo<textarea value={draft.objective || ""} onChange={(event) => update("objective", event.target.value)} /></label><label>Meta<textarea value={draft.goal || ""} onChange={(event) => update("goal", event.target.value)} /></label><label>Incluye (separado por comas)<textarea value={(draft.scope_items || []).join(", ")} onChange={(event) => update("scope_items", event.target.value.split(",").map((item) => item.trim()).filter(Boolean))} /></label><label>Limitaciones (separado por comas)<textarea value={(draft.limitations || []).join(", ")} onChange={(event) => update("limitations", event.target.value.split(",").map((item) => item.trim()).filter(Boolean))} /></label><div className="ph-manager__inline"><label>Inicio<input type="date" value={draft.start_date || ""} onChange={(event) => update("start_date", event.target.value)} /></label><label>Entrega<input type="date" value={draft.estimated_end_date || ""} onChange={(event) => update("estimated_end_date", event.target.value)} /></label></div><label>Orden<input type="number" value={draft.sort_order ?? 0} onChange={(event) => update("sort_order", Number(event.target.value))} /></label></> : null}
