...
const sections = [
  { id: "project_solutions", label: "1 · Soluciones" },
  { id: "project_solution_brief_versions", label: "2 · Ficha" },
...
const statusLabel = { draft: "Borrador", active: "Activo", planned: "Programado", at_risk: "Atrasado", on_hold: "En espera", waiting: "En espera", needs_changes: "Requiere cambios", completed: "Terminado", cancelled: "Cancelado", backlog: "Por iniciar", todo: "Por hacer", pending: "Pendiente", in_progress: "En proceso", review: "En revisión", done: "Terminada", blocked: "En espera", delivered: "Entregado", approved: "Aprobado", rejected: "Requiere cambios", resolved: "Resuelto", accepted: "Aceptado", not_applicable: "No aplica", superseded: "Reemplazada" };
const childTitle = (item) => item.name || item.title || item.problem || item.description || (item.minutes ? `${item.minutes} min · ${item.work_date}` : "") || (item.activity_date ? `${item.lines_added} líneas · ${item.activity_date}` : "") || `Ficha v${item.version_number || 1}`;
const confirmedProgress = (project) => {
  const items = project?.project_deliverables || [];
  const total = items.reduce((sum, item) => sum + Number(item.weight || 1), 0);
  const accepted = items.reduce((sum, item) => sum + (item.status === "approved" ? Number(item.weight || 1) : 0), 0);
    ...
    <section className="ph-manager">
      ...
      <div className="ph-manager__layout">
        <aside className="ph-manager__projects">
          ...
          {projects.map((project) => <button className={selectedId === project.id ? "active" : ""} type="button" key={project.id} onClick={() => selectProject(project)}><strong>{project.name}</strong><span>{statusLabel[project.status]} · {confirmedProgress(project)}% confirmado</span><small>{project.published ? "Visible para cliente" : "Borrador interno"}</small></button>)}
        ...
        <div className="ph-manager__content">
          <article className="ph-manager__card">
            ...
            <div className="ph-manager__form-grid">
              ...
              <label>Avance confirmado<input value={`${confirmedProgress(projectDraft)}% · calculado por entregables aceptados`} disabled /></label>
...
function ChildForm({ table, draft, setDraft, project, saving, onSave, onCancel }) {
  ...
  return <div className="ph-manager__child-form"><h4>{draft.id ? "Editar registro" : "Agregar registro"}</h4>
    ...
    {table === "project_objectives" ? <><label>Nombre del periodo<input value={draft.period_label || ""} placeholder="Ej. 10–21 agosto" onChange={(event) => update("period_label", event.target.value)} /></label><div className="ph-manager__inline"><label>Inicio<input type="date" value={draft.period_start || ""} onChange={(event) => update("period_start", event.target.value)} /></label><label>Fin<input type="date" value={draft.period_end || ""} onChange={(event) => update("period_end", event.target.value)} /></label></div><label>Estado<select value={draft.status} onChange={(event) => update("status", event.target.value)}><option value="planned">Programado</option><option value="active">En curso</option><option value="completed">Completado</option><option value="at_risk">En riesgo</option></select></label><label>Orden<input type="number" value={draft.sort_order ?? 0} onChange={(event) => update("sort_order", Number(event.target.value))} /></label></> : null}
